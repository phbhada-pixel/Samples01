  ...
  try {
    ...
    switch (method) {
      ...
      case 'getMonthlyReportDashboardData': {
        ...
        let monthObj = null;
        if (targetMonth) {
          monthObj = monthMaster.find(m => clean(m.name) === clean(targetMonth));
        }
        if (!monthObj) {
          const today = new Date();
          today.setHours(0, 0, 0, 0);
          monthObj = monthMaster.find(m => today >= m.f1Start && today <= m.f2End) || monthMaster[8] || monthMaster[0];
        }

        const targetIdx = monthMaster.indexOf(monthObj);
        let priorOpdSum = 0;
        let priorFeverSum = 0;
